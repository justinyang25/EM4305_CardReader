...
{
		unsigned char x,y;
		unsigned char xdata buff_x[8] = {0};	
	  union 
		{
				unsigned long uL;					
				unsigned char B[4];
	  ...
	  Write_DataManage();
		...
		ReadWriteEM4305(WriteCom,4,0x5F800100,64);
		Delay_8nus(6250);
		for(x = 0;x < 4;x ++) 	BankData.B[x] = buff_x[x];
		ReadWriteEM4305(WriteCom,5,BankData.uL,64);
		Delay_8nus(6250);
		for(x = 0;x < 4;x ++) 	BankData.B[x] = buff_x[4 + x];
		ReadWriteEM4305(WriteCom,6,BankData.uL,64);
		Delay_8nus(6250);
}
...
void T5557CopyEM4100(unsigned char *buf)
{
		CopyEM4100Uid(buf);
	  Write_DataManage();
	  Standard_Write(0,E5557_Config);	    //写配置块
	  Delay_8nus(1250);
	  Standard_Write(1,Write_Buf[0]);			//写0页第一块
	  Delay_8nus(1250);
	  Standard_Write(2,Write_Buf[1]);			//写0页第二块		
		Delay_8nus(1250);	
}